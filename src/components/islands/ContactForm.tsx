...
const validate = (currentValues: FormValues) => {
	...
	if (!currentValues.age) {
		...
		nextErrors.age = "La edad debe estar entre 0 y 120 años.";
	}

	const cleanPhone = normalizePhone(currentValues.phone);
	if (!cleanPhone) {
		nextErrors.phone = "Ingresa un número de teléfono.";
	} else if (cleanPhone.length < 8) {
		nextErrors.phone = "El teléfono debe tener al menos 8 dígitos.";
	}

	if (!currentValues.service.trim()) {
		nextErrors.service = "Selecciona un servicio de interés.";
	}
...
export default function ContactForm({
	...
	const handleSubmit = (event: React.FormEvent<HTMLFormElement>) => {
		...
		const whatsappMessage = [
			"Hola, deseo agendar una cita en Servicios Médicos ROMAT.",
			"",
			"Datos del paciente:",
			`- Nombre completo: ${values.fullName.trim()}`,
			`- Edad: ${values.age.trim()} años`,
		...
		<form onSubmit={handleSubmit} className="space-y-4" noValidate>
			...
			<div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
				<p className="text-xs text-muted-foreground">
					* Campos obligatorios para enviar la solicitud.
				</p>
				<button
					type="submit"
